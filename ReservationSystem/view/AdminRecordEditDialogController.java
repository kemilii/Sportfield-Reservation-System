...
public class AdminRecordEditDialogController {
	@FXML
    private TextField classOrSubjectTextField;
	@FXML
    ...
    public void conductRecord(Record selectedRecord) {
    	...
    	oldRecord.setClassOrSubject(selectedRecord.getClassOrSubject());
    	oldRecord.setName(selectedRecord.getName());
    ...
    public void setRecord() {
    	classOrSubjectTextField.setText(oldRecord.getClassOrSubject());
        nameTextField.setText(oldRecord.getName());
    ...
    private void handleOk() throws SQLException {
        if (isInputValid()) {
        	newRecord.setClassOrSubject(classOrSubjectTextField.getText());
    ...
    private boolean isInputValid() throws SQLException {
    	String classOrSubject = classOrSubjectTextField.getText();
    	...
    	if(classOrSubject == null || name == null || sport == null || place == null || date == null || time == null) {
    		errorMessage += "There is empty field!";
    	}
    	else if(classOrSubject.trim().length() <= 0 || name.trim().length() <= 0 || sport.trim().length() <= 0 || place.trim().length() <= 0 
    			|| date.trim().length() <= 0 || time.trim().length() <= 0) {
        	errorMessage += "There is empty field!";
        }
    	else{
    		User user = new User(classOrSubject, name);
    		if(!UserDao.verifyUser(user)) {
    			errorMessage += "There is incorrect 'class or subject' or name!";
    		}
